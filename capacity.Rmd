...
The analysis below was done on 43 responses from MLIS team members. One of the key purpose was to test MLIS team members understanding in several key topics listed below:
...
* Learning 
* Data Collection  
* Qualitative and Quantitative Data management, analysis, and visualization packages 
* Information Systems (IS) 

***
The overall rating of MLIS knowledge in the above topics is graphed below.with “1” meaning Not knowledgeable at all, “2” Less knowledgeable, “3” A little more Knowledgeable, “4” “Sufficient Knowledge, and “5” Excellent knowledge. **Kindly hover above all graphs in this document to see more details**

```{r echo=FALSE}
overall_score <-
  read.csv("~/GabuTheGreat.github.io/overall_Score.csv")


p = ggplot(overall_score,
           aes(Knowledge_Area, MLIS_Score, fill = Knowledge_Area)) +
...
div(ggplotly(p) %>% config(displayModeBar = F) %>% layout(legend = list(x = 1, y = 1)),
    align = "center")
```
...
div(ggplotly(p) %>% config(displayModeBar = F) %>% layout(legend = list(x = 0, y = 1)), align = "center")
...
div(ggplotly(p) %>% config(displayModeBar = F) %>% layout(legend = list(x = 1, y = 1)),align = "center")
...
***
### All units Avarage scores in Research.

```{r echo=FALSE}
research_plot <- select(avarage_research, unit, Mean)

p = ggplot(research_plot, aes(unit, Mean, fill = unit)) +
  geom_bar(stat = "identity") +
...
***
### All units Avarage scores in Monitoring and Evaluation.
```{r echo=FALSE}
mne_plot <- select(avarage_mne, unit, Mean)

p = ggplot(mne_plot, aes(unit, Mean, fill = unit)) +
  geom_bar(stat = "identity") +
  geom_text(aes(label = Mean), fontface = "bold") +
  theme(
    axis.text.x = element_blank(),
...
***
### All units Avarage scores in Learning.
```{r echo=FALSE}

learning_plot <- select(avarage_learning, unit, Mean)

p = ggplot(learning_plot, aes(unit, Mean, fill = unit)) +
  geom_bar(stat = "identity") +
  geom_text(aes(label = Mean), fontface = "bold") +
  theme(
    axis.text.x = element_blank(),
...
***
### All units Avarage scores in Data Collection.
```{r echo=FALSE}
dcollection_plot <- select(avarage_dcollection, unit, Mean)

p = ggplot(dcollection_plot, aes(unit, Mean, fill = unit)) +
  geom_bar(stat = "identity") +
  geom_text(aes(label = Mean), fontface = "bold") +
  theme(
    axis.text.x = element_blank(),
...
***
### All units Avarage scores in Qualitative and Quantitative Data management, analysis, and visualization packages.

```{r echo=FALSE}
qqAnalysis_plot <- select(avarage_qqAnalysis, unit, Mean)

p = ggplot(qqAnalysis_plot, aes(unit, Mean, fill = unit)) +
  geom_bar(stat = "identity") +
  geom_text(aes(label = Mean), fontface = "bold") +
  theme(
    axis.text.x = element_blank(),
...
***
### All units Avarage scores in Information Systems (IS).

```{r echo=FALSE}
IFSystems_plot <- select(avarage_IFSystems, unit, Mean)

p = ggplot(IFSystems_plot, aes(unit, Mean, fill = unit)) +
  geom_bar(stat = "identity") +
  geom_text(aes(label = Mean), fontface = "bold") +
  theme(
    axis.text.x = element_blank(),
...
***
### **2.4) Additional Training Requirement Listed By MLIS team Members **

Members of Field Monitoring team listed most of the additional training required, more than any other MLIS teams.

```{r echo=FALSE}

FMT_Training <- read.csv("~/GabuTheGreat.github.io/training_fmt.csv")
kable(FMT_Training, caption = "Additional Training Required by FMT team") %>% kable_styling(bootstrap_options = c("striped", "hover", "condensed"))

```

***
The other teams requisted additional training as tabulated below.

```{r echo=FALSE}
FMT_Training <- read.csv("~/GabuTheGreat.github.io/training_other.csv")
kable(FMT_Training, caption = "Additional Training Required other MLIS teams") %>% kable_styling(bootstrap_options = c("striped", "hover", "condensed"))
```


### **2.5) Analysis on Other Software(s)Listed by MLIS to strengthen their skills **

We asked MLIS members what other data management, Information systems, data analysis and data visualization software they would wish to strengthen their skills in, to enable them to perform bette. 

#### **I. Other Data Collection software(s)**
We asked MLIS team members to list other data collection softwares they would like to strengthen their skills in. Their responses are tabuleted below per unit.

```{r echo=FALSE}
Other_dataCollection <-
  cleaned_data %>% select(unit, other_dcs) %>% 
  filter(other_dcs != "") %>% group_by(unit) %>%
  summarise(Software = toString(unique(other_dcs)))

kable(Other_dataCollection, caption = "Other Data Collection Softwares to Stregthen Skills In") %>% kable_styling(bootstrap_options = c("striped", "hover", "condensed"))
```

#### **II. Other Data management software(s)**

Other Data management software MLIS team members would like to stregthen their skills in.

```{r echo=FALSE}
Other_dataManagement <-
  cleaned_data %>% select(unit, other_dms) %>% 
  filter(other_dms != "" & other_dms != "N/A" & other_dms != "None") %>% group_by(unit) %>%
  summarise(Software = toString(unique(other_dms)))

kable(Other_dataManagement, caption = "Other Data Management Softwares to Stregthen Skills In") %>% kable_styling(bootstrap_options = c("striped", "hover", "condensed"))
```

#### **IV. Other Information Systems software(s).**

Other Information Systems software MLIS team members would like to stregthen their skills in.

```{r echo=FALSE}
Other_informationIS <-
  cleaned_data %>% select(unit, other_Is) %>% 
  filter(other_Is != "" & other_Is != "N/A" & other_Is != "None") %>% group_by(unit) %>%
  summarise(Software = toString(unique(other_Is)))

kable(Other_informationIS, caption = "Other Information System Softwares to Stregthen Skills In") %>% kable_styling(bootstrap_options = c("striped", "hover", "condensed"))
```

#### **V. Other Data Analysis software(s).**

Other Data Analysis software MLIS team members would like to stregthen their skills in.

```{r echo=FALSE}
Other_dataanalysis <-
  cleaned_data %>% select(unit, other_da) %>% 
  filter(other_da != "" & other_da != "N/A" & other_da != "None") %>% group_by(unit) %>%
  summarise(Software = toString(unique(other_da)))

kable(Other_dataanalysis, caption = "Other Data Analysis Softwares to Stregthen Skills In") %>% kable_styling(bootstrap_options = c("striped", "hover", "condensed"))
```

#### **VI. Other Data Visualization software(s).**

Other Data Visualization software MLIS team members would like to stregthen their skills in.

```{r echo=FALSE}
Other_dataVizual <-
  cleaned_data %>% select(unit, other_dv) %>% 
  filter(other_dv != "" & other_dv != "N/A" & other_dv != "None"& other_dv != "Tableu;Cluvio") %>% group_by(unit) %>%
  summarise(Software = toString(unique(other_dv)))

kable(Other_dataVizual, caption = "Other Data Vizualization Softwares to Stregthen Skills In") %>% kable_styling(bootstrap_options = c("striped", "hover", "condensed"))
```

### **2.6) Analysis on Knowledge and skills in Research Monitoring**

All partcipants were asked to rate their skills in research and monitoring. The results of the analysis of their responses his tabulated below.
...
table_kwld  = arrange(table_kwld , desc(count))
kable(table_kwld , caption = "Knowledge and Skills in  Research and Monitoring") %>%  kable_styling(bootstrap_options = "striped", full_width = F)
```

### **2.7) MLIS Team Members in Need of an Extra Tool(s) **

MLIS team members were asked if there is any tools they would wish to obtain in order to enable them perform their work better.
Sixt nine percent **(69 percent)** of MLIS members listed that, they would need to acquire a certain tool to enable them work better.
...
table_tools  = arrange(table_tools, desc(count))
kable(table_tools, caption = "Percentage of MLIS Team Members in Need of a Tool(s)") %>%  kable_styling(bootstrap_options = "striped", full_width = F)
```

### **2.8) Tools listed by MLIS Members **

We analysed the responses of MLIS team members, and the table below lists tools requsted per unit.
...
kable(tools_requested, caption = "Tools Required Per Unit") %>% kable_styling(bootstrap_options = c("striped", "hover", "condensed"))

```

### **2.9) Other listed tools **

We asked MLIS members to list tools they would require if they were noted listed in the previous querstion.
...
kable(Othertools_requested, caption = "Tools Required Per Unit") %>% kable_styling(bootstrap_options = c("striped", "hover", "condensed"))
```

### **3.0) Final Remarks **

We asked MLIS team members to list their final remarks concerning the whole exercise. This is what they listed down.

```{r echo=FALSE}
remarks <- cleaned_data %>% select(remarks) %>% 
           filter(remarks != "N/A" & remarks != "None" & remarks != "Not applicable" & remarks != "N/a" & remarks != "")

kable(remarks, caption = "Final Remarks by MLIS Team Members") %>% kable_styling(bootstrap_options = c("striped", "hover", "condensed"))
```
